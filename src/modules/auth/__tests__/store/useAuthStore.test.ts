...
describe('useAuthStore', () => {
  ...
  describe('fetchUser', () => {
    ...
    it('should handle error when fetching user', async () => {
      ...
      // Mock Supabase getSession with error
      vi.mocked(supabase.auth.getSession).mockResolvedValue({
        data: { session: null },
        error: mockError
      })
  ...
  describe('signIn', () => {
    ...
    it('should handle error when signing in', async () => {
      ...
      // Mock Supabase signInWithPassword with error
      vi.mocked(supabase.auth.signInWithPassword).mockResolvedValue({
  ...
  describe('sendOTP', () => {
    ...
    it('should handle error when sending OTP', async () => {
      ...
      // Mock Supabase signInWithOtp with error
      vi.mocked(supabase.auth.signInWithOtp).mockResolvedValue({
        data: { user: null, session: null },
        error: mockError
      })
  ...
  describe('verifyOTP', () => {
    ...
    it('should throw error when no verification is pending', async () => {
      ...
      await expect(store.verifyOTP('123456')).rejects.toThrow('Aucune vérification en attente')
    })

    it('should handle error when verifying OTP', async () => {
      const mockError = createMockAuthError('Invalid OTP')

      // Mock Supabase verifyOtp with error
      vi.mocked(supabase.auth.verifyOtp).mockResolvedValue({
        data: { user: null, session: null },
        error: mockError
      })
